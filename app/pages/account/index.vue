...
<template>
  <div class="justify-content-center align-content-center display: flex flex-wrap fill-height mt-8">
    <div class="surface-card p-4 shadow-2 border-round md:w-6 lg:w-4">
      ...
      <div style="height: 350px" class="border-2 border-dashed surface-border"> <pre>{{ user }}</pre></div>
      <Button v-if="user" label="Logout" icon="pi pi-user" class="w-half mt-3" @click="logout"></Button>
      <Button v-if="!user" label="Login" icon="pi pi-user" class="w-half mt-3 ml-3" @click="navigateTo('/auth/login')"></Button>
    </div>
  </div>
</template>
