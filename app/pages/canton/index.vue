...
<template>
  <div class="justify-content-center align-content-center display: flex flex-wrap fill-height mt-5">
    <DataTable v-model:filters="filters" :value="records" paginator :rows="10" dataKey="id" filterDisplay="row" :loading="loading"
                :globalFilterFields="['name','expand.association.name']">
            <template #header>
            ...
            <template #empty> Keine Kantone gefunden. </template>
            <template #loading> Kantone werden geladen. Bitte warten. </template>
            ...
            <Column field="association" header="Verband" filterField="expand.association.name" style="min-width: 12rem">
                <template #body="{ data }">
                    {{ data.expand.association.name }}
                </template>
                <template #filter="{ filterModel, filterCallback }">
                    <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" placeholder="Filter Verband" />
                </template>
            </Column>
        </DataTable>
  </div>
